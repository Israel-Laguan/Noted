...
class Command(StaticfilesRunserverCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not User.objects.filter(username="demo@noted.app").exists():
            ...
            Note.objects.create(owner=user, category=work, title="Welcome to Noted", content="This is a demo note. Edit or delete it to get started!")
            Note.objects.create(owner=user, category=personal, title="Shopping List", content="Milk\nEggs\nBread")
            Note.objects.create(owner=user, category=ideas, title="Project Ideas", content="A note-taking app with AI assistance\nA recipe organizer\nA habit tracker")
