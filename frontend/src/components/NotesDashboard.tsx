  ...
  return <main className="flex h-screen flex-col overflow-hidden pl-6 pr-9 max-sm:px-4">
    ...
    <div className="mt-5 flex min-h-0 flex-1 gap-8 max-sm:flex-col max-sm:gap-3">
      <aside className="flex w-64 shrink-0 flex-col max-sm:flex max-sm:w-full max-sm:overflow-x-auto max-sm:overscroll-x-contain max-sm:scroll-smooth max-sm:[-ms-overflow-style:none] max-sm:[scrollbar-width:none] max-sm:[&::-webkit-scrollbar]:hidden">
        ...
        <button className="mt-3 flex w-full cursor-pointer items-center gap-3 rounded-full px-3 py-2 text-[14px] font-bold text-line hover:bg-red-500/10" onClick={() => setShowLogoutModal(true)} title="Log out">
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-line font-bold text-white">{user?.first_name?.[0]?.toUpperCase() || user?.email[0].toUpperCase()}</span>
          <span>Logout</span>
        </button>
        <LogoutModal open={showLogoutModal} onConfirm={logout} onCancel={() => setShowLogoutModal(false)} />
      </aside>
